...
#include <asm/hardware.h>
extern unsigned long omap_mpu_timer_ticks_to_usecs(unsigned long);

#define xip_irqpending()	\
	(omap_readl(OMAP_IH1_ITR) & ~omap_readl(OMAP_IH1_MIR))
#define xip_currtime()		(~omap_mpu_timer_read(0))
#define xip_elapsed_since(x)	\
	(signed)omap_mpu_timer_ticks_to_usecs(~omap_mpu_timer_read(0) - (x))
